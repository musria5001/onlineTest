...
from work.ExcelDealer import ExcelDealer
...
from django.http import HttpResponse, StreamingHttpResponse, FileResponse, JsonResponse, Http404
...
def show_homework_result(request, id=0):
    ...
    if homework_answer.homework.creater == request.user or request.user.is_superuser:
        pass
    else:
    ...
    for banji in homework.banji.all():
        ...
        if homework_answer.creator in students:
            ...
            break
    homework_answers = homework_answers.filter(creator__banJi_students__in=BanJi.objects.filter(id__in=banjiList))
    choice_problems = []
    ducheng_problems = []
    problems = []
    tiankong_problems = []
    ...
    for info in json.loads(homework.choice_problem_info):  # 载入作业的选择题信息，并进行遍历
        if str(info['id']) in wrong_id:  # 如果答案有错
            wrong_info += ['未回答']*(len(wrong_id)-len(wrong_info))
            choice_problems.append(
                {'id': info['id'], 'detail': ChoiceProblem.objects.get(pk=info['id']), 'right': False,
                 'info': wrong_info[wrong_id.index(str(info['id']))]})
        else:  # 如果答案正确
            choice_problems.append(
                {'id': info['id'], 'detail': ChoiceProblem.objects.get(pk=info['id']), 'right': True})
    ...
    for pid in problem_ids:
        ...
        similar_code_owners = []
        try:
            solution = Solution.objects.filter(problem_id=pid,homework_answer=homework_answer)
            if len(solution)==0:
                continue
            else:
                solution = solution[0]
    ...
    context={'choice_problems': choice_problems, 'problem_score': homework_answer.problem_score,
                       ...
                       'title': ' {}({})的"{}"详细'.format(homework_answer.creator.username,homework_answer.creator.id_num,homework.name),
                       'show_answer': show_answer, 'remained_number': remained_number,
                       'resubmit_number': resubmit_number, 'is_end': is_end,
                       'student_detail': homework_answer.student_detail}
...
def do_homework(request, homework_id=0):
    ...
    if not request.user.is_superuser:
        if request.user.is_teacher:
            # homeworks = MyHomework.objects.filter(creater=request.user).all()
            if homework.creater!=request.user:
                raise Http404()
        else:
    ...
    if request.method == 'POST':  # 当提交作业时
        ...
        if not homework.allow_resubmit:
            if request.user in homework.finished_students.all():  # 防止重复提交
                logger.info(log + "，执行结果：失败（不允许重复提交）")
            ...
            # 如果允许重复提交，需要进一步判断是否以前曾经有过作业记录
            homeworkAnswers = HomeworkAnswer.objects.filter(creator=request.user, homework=homework)
            if homeworkAnswers:
                # 如果有过作业记录，需要删除相关的作业信息
                homeworkAnswer = homeworkAnswers[0]
                if homeworkAnswer.remained_number>=homework.resubmit_number:
                    logger.info(log + "，执行结果：失败（到达提交次数上限）")
                    return render(request, 'warning.html', context={'info': '提交失败，您已达到提交次数上限'})
        ...
        if homework.ducheng_problem_ids:
            ...
            for id in homework.ducheng_problem_ids.split(','):
                ...
                if id and request.POST.get('ducheng-' + id) not in (DuchengProblem.objects.get(pk=id).answer).split('|||'):  
                    ...
                    if request.POST.get('ducheng-' + id) is None or request.POST.get('ducheng-' + id)=="":
        ...
        for k, v in request.POST.items():
            ...
            if k.startswith('source'):
                ...
                if 'solution-'+k[7:] in request.POST.keys():
                    try:
                        solution = Solution.objects.get(solution_id=request.POST['solution-'+k[7:]])
                        # todo:根据solution取code，判定code是否与v相同
                    except ObjectDoesNotExist:
                        needNewSolution = True
                    except ValueError:
                        needNewSolution = True
                    except:
                        logger_request.exception("执行动作：读取Solution，用户信息：{}({}:{})，POST数据：{}".format(request.user.username,request.user.pk,request.user.id_num,request.POST.dict()))
                else:
                    needNewSolution = True

                while needNewSolution :
                    if 'language-'+k[7:] in request.POST.keys():
                        solution_language = request.POST['language-' + k[7:]]
                    else:
                        solution_language = homework.allowed_languages.split(",")[0]
                    try:
                        # 否则，创建新的Solution数据
                        code = v if v else  "未回答"
                        solution = Solution(problem_id=k[7:], user_id=request.user.username,
                                language=solution_language, ip=request.META['REMOTE_ADDR'],
                                code_length=len(v))
                        ...
                        logger_request.exception("执行动作：创建Solution，用户信息：{}({}:{})，POST数据：{}".format(request.user.username,request.user.pk,request.user.id_num,request.POST.dict()))
                        time.sleep(1)
        ...
        if "summary" in request.POST.keys():
            homeworkAnswer.summary = request.POST['summary']
        if "student_detail" in request.POST.keys():
            if homeworkAnswer.student_detail:
                homeworkAnswer.student_detail += request.POST['student_detail']
            else:
                homeworkAnswer.student_detail = request.POST['student_detail']
        homeworkAnswer.save()
        ...
        try:  # 如果有暂存的该作业答案，删除掉
            ...
            TempHomeworkAnswer.objects.filter(creator=request.user, homework=homework).delete()
            logger.info(log + "，执行结果：成功")
        except:
            raise
        ...
        if homework.allow_random:
            ...
            # 用于将题目中的选项进行乱序
            for xxx in choice_problems:
                list_choices=[xxx['detail'].a,xxx['detail'].b,xxx['detail'].c,xxx['detail'].d]
                list_choices_id=['a','b','c','d']
                old_dict_choices=dict(zip(list_choices_id,list_choices))
                random.shuffle(list_choices)
                new_dict_choices=dict(zip(list_choices_id,list_choices))
                dict_new_old={}
                for opti in list_choices_id:
                    dict_new_old[opti]=list(old_dict_choices.keys())[list(old_dict_choices.values()).index(new_dict_choices[opti])]
                xxx['detail'].a=list_choices[0]
                xxx['detail'].b=list_choices[1]
                xxx['detail'].c=list_choices[2]
                xxx['detail'].d=list_choices[3]
                xxx['opti_new_old']=dict_new_old.copy()
        else:
            for problem in choice_problems:
                dict_choices={'a':'a','b':'b','c':'c','d':'d'}
                problem['opti_new_old']=dict_choices.copy()
        ...
        try:
            ...
            tempHomeworkAnswer = TempHomeworkAnswer.objects.filter(creator=request.user, homework=homework)[0]
        # 对于限时完成的作业，计算剩余时间
        if homework.time_allowed == -1 or homework.time_allowed is None:
            # 判断用户是否在允许的时间内打开当前作业
            if not (homework.start_time<datetime.datetime.now()<homework.end_time) and \
                    homework.creater!=request.user:
                logger.info(log + "，执行结果：失败（时间不允许）")
                return render(request, 'warning.html', context={'info': '当前作业不在允许的时间范围内！'})
        ...
        return render(request, 'do_homework.html',
                    context={'homework': homework, 'problemsType': ['编程题','程序填空题','程序改错题'],
                             ...
                             'remained_number': remained_number})
...
def ajax_add_students(request):
    ...
    if request.POST['judge']=='1':
        ...
        if len(st) > 1:
            ...
            if teacher.school == '' and teacher.school_short == '':
                try:
                    if type(id_num)==float : id_num=str(int(id_num))
                    if type(id_num)==int : id_num=str(id_num)
                    student = MyUser.objects.get(id_num=id_num)
                    if student.username == username:
                        student.groups.add(Group.objects.get(name='学生'))
                    elif username in student.username:
                    ...
                    student.set_password(id_num + '@Njupt')
                    student.save()
                    student.groups.add(Group.objects.get(name='学生'))
            else:
                try:
                    student = MyUser.objects.get(id_num=teacher.school_short+str(id_num))
                    if student.username == username:
                        student.groups.add(Group.objects.get(name='学生'))
                    elif username in student.username:
                        student.username = username
    ...
    if request.POST['judge']=='0':
        ...
        if len(stu_detail.split()) > 1:
            ...
            return HttpResponse(json.dumps({'result': 0, 'count': 0, 'allow': 1,'message':'信息不完整，至少应该包含学号和姓名'}))
        banji = BanJi.objects.get(pk=banji_id)
        if teacher.create_num > 0:
            if student not in banji.students.all():
                banji.students.add(student)
...
def reset_stupassword(request):
    if request.method == 'POST' and request.user.isTeacher():
        ...
        if (student.school_short is not None) and student.school_short != '':
            student.set_password(stu_nu)
        else:
            student.set_password(stu_nu + '@Njupt')
        student.save()
        return HttpResponse(1)
    else:
        raise Http404()
    ...
    for homework in homeworks.all().order_by(sort):
        ...
        if (request.user in homework.finished_students.all()):
            ...
            if homework_answers.exists():
                ...
                # 如果作业不存在，大概率是创建作业的时候遇到了问题，所以需要将学生从完成列表中移除
                homework.finished_students.remove(request.user)
...
def get_problem_score(homework_answer, judged_score=0):
    ...
    for info in json.loads(homework.problem_info):
        ...
        try:
            ...
            logger_request.exception("1463行发生异常")
    return score
...
def list_finished_homework(request):
    ...
    return render(request, 'finidshed_homework_list.html',
                  context={'classnames': banjis, 'position': 'finished'})
                ...
                if len(maxscore)>0:
                    ...
                    for k in range(1, c_length):
                        for i in range(0, r_length):
                            if student.id_num in sheet2.cell_value(i, 0):
                                if sheet2.cell_value(i, k) != '无' and sheet2.cell_value(i, k) != '-':
                                    moocScore = moocScore + float(sheet2.cell_value(i, k))
                                break
                    record['moocScore'] = '%.1f' % moocScore
                    average_mscore = 0.0
                    s_score=[]
                    for row in range(0, r_length):
                        if student.id_num in sheet2.cell_value(row, 0):
                            ...
                            s_score=[0 if i=='无' or i=='-' else i for i in s_score]
                            s_score=[float(x) for x in s_score]
                            s_score=list(map(lambda x,y:x/y,s_score,maxscore))
                            sum_score=0.0
                            for score in s_score:
                                sum_score = sum_score + score
                            average_mscore = sum_score/len(s_score)
                # 计算作业成绩
                total_pscore = 0
                average_pscore = 0.0
                for index, homework in enumerate(homeworks):
                    answers = homework.homeworkanswer_set
...
def student_score(request):
    ...
    for banji in banjis:
        ...
        last_score = None
        try:
            if banji.work_result is None:
                continue
            work_result = eval(banji.work_result)
        except ValueError:
            continue
        if 'rows' not in work_result.keys():
            continue
        records = work_result['rows']
        records.sort(key=lambda record:0 if 'final_score' not in record.keys() else float(record['final_score']), reverse=True)
        for record in records:
            if 'final_score' not in record:
                continue
            if last_score is None or record['final_score']==last_score:
                count += 1
            else:
                rank += count
                count = 1
            last_score = record['final_score']

            if record['id'] == request.user.id_num:
                item['banji'] = banji.name
                if 'moocScore' in record.keys():
                    item['mscore'] = record['moocScore']
                else:
                    item['mscore'] = 0
                if 'total_pscore' in record.keys():
                    item['pscore'] = record['total_pscore']
                else:
                    item['pscore'] = 0
                if 'final_score' in record.keys():
                    item['fscore'] = record['final_score']
                else:
                    item['fscore'] = 0
                item['sign'] = str(record['signedCount']) + '/' + str(record['signCount'])
                item['rank'] = rank
                break
        lst.append(item)
    return render(request, 'my_score.html', context={'banjis': banjis, 'title': '我的数据', 'data':lst})
...
def get_finished_students(request):
    ...
    if 'search' in request.GET.dict().keys():
        qset = ( Q(creator__username__icontains=request.GET['search']) |
                 Q(creator__id_num__icontains=request.GET['search']) )
        homework_answers = homework_answers.filter(qset)
    if 'sort' in request.GET.dict().keys():
        sort = request.GET['sort']
    homework_answers = homework_answers.filter()
    json_data['total'] = homework_answers.count()
    if 'order' in request.GET.dict().keys() and request.GET['order'] == 'desc':
        sort = '-' + sort
...
def judge_homework(homework_answer):
    ...
    :return:None
    """
    loginfo = "执行动作：计算成绩，用户信息：{}({}:{})，作业ID：{}，".format(
      homework_answer.creator.username,homework_answer.creator.pk,homework_answer.creator.id_num,homework_answer.homework)
    logger.info(loginfo + "开始计算...")
    for i in range(1,61):
        if i % 20==0:
        ...
        for solution in homework_answer.solution_set.all():  # 遍历作业的solution集合
            print("对SolutionID为{}的程序进行检查：".format(solution.solution_id),end="")
            ...
            logger.info(loginfo+ "尝试次数：{}，总分：{}(选择题：{}，读程题：{}，编程题：{}，程序填空题：{}，程序改错题：{})，执行结果：成功".format(i,zongfen,choice_problem_score,ducheng_problem_score,biancheng_score,tiankong_score,gaicuo_score))
            break  # 跳出循环
    else:
        logger.info(loginfo+ "执行结果：失败")
...
def test_run(request):
    ...
    if request.POST['type'] == 'upload':  # 当上传代码时
        log = "执行动作：上传程序，用户信息：{}({}:{})，表单信息：{}".format(request.user.username,request.user.pk,request.user.id_num,request.POST.dict())
        if {'problem_id','language','code'} - set(request.POST.dict()) != set():
            logger.info(log + "，执行结果：失败（缺失必要信息）")
            raise Http404()
        try:
            ...
            # 创建失败，返回错误信息
            logger.info(log + "，执行结果：失败（创建解决方案失败）")
            return HttpResponse(json.dumps({'result': 0, 'info': '出现了问题' + e.__str__(), 'score': 0}))
            
    if request.POST['type'] == 'score':  # 当获取结果时
        log = "执行动作：获取程序测试结果，用户信息：{}({}:{})，表单信息：{}".format(request.user.username,request.user.pk,request.user.id_num,request.POST.dict())
        ...
        if solution.result in [0, 1, 2, 3]:  # 当题目还在判断中时
            logger.info(log + "，执行结果：成功（程序正在判断中）")
            return HttpResponse(json.dumps({'status': 0, 'info': '程序正在判断中', 'score': 0}))
                ...
                if info['pk'] == solution.problem_id:
                    for case in info['testcases']:  # 获取题目的测试分数
                        try:
                            ...
                            logger.info(log + "，执行结果：失败（计算程序得分出错）")
                            logger_request.exception("Exception Logged:{homework_id:"+str(homework.id)+",solution.problem_id:"+str(solution.problem_id)+",solution.oi_info:"+str(json.loads(solution.oi_info))+"}")
            #2017年3月22日，注释以下两句，保留学生做作业的过程
            #SourceCode.objects.get(solution_id=solution.solution_id).delete()
            #solution.delete()
            logger.info(log + "，执行结果：成功（本题正确率：{}/{}）".format(right_num,len(cases)))
...
def delete_homeworkanswer(request, id):
    ...
    for solution in homeworkanswer.solution_set.all():
        ...
        solution.delete()
    # 如果有暂存的该作业答案，删除掉
    TempHomeworkAnswer.objects.filter(creator=request.user, homework=homeworkanswer.homework).delete()
...
def rejudge_homework(request, id):
    ...
    for i in homework_answer.solution_set.all():
        i.result = 0
        i.save()
        print('将Solution('+str(i.solution_id)+')重置')
...
def comment_change(request):
    ...
    try:
        ...
        # print(dir(homework_answer))
        if homework_answer[0].homework.creater != request.user and (not request.user.is_superuser):
            raise Http404()
        if eval(change) == -1:
        ...
        logger_request.exception("2013行发生异常")
    return HttpResponse(1)
...
def once(request):
    ...
    if request.method == "POST":
        if request.FILES.get('names'):
            ...
            uploadedFile = request.FILES.get('names')
            if not uploadedFile:
                return render(request, 'once.html', {'msg':'错误没有选择文件','class_id':class_id})
            filename = classname + "_upload.xls"
            # filename = uploadedFile.name
            dstFilename = join(uploadDir, filename)
            with open(dstFilename, 'wb') as fp:
                for chunk in uploadedFile.chunks():
                    fp.write(chunk)

            banji = BanJi.objects.filter(pk=int(classname))
            ...
            j=2 # 第一行留下来存放来源数据的文件名
            # 获取满分
            # max_score = sheet1.cell_value(0, 4)[3:]
            max_score = ''
            length2 = data_sheet1.ncols if data_sheet1.ncols>0 else 1

            # 在第一行添加数据来源文件名
            data_source_name = {}
            for col in range(1, length2):
                if data_sheet1.cell_value(0,col) is not None and data_sheet1.cell_value(0,col) != "":
                    data_source_name[str(data_sheet1.cell_value(0,col))] = col
            if filename in data_source_name:
                length2 = data_source_name[filename]
            data_sheet2.write(0, length2, filename)

            data_sheet2.write(1, length2, max_score)
            stuDict = {}
            ...
            for i in range(sheet1.ncols):
                if '昵称' in sheet1.cell_value(0,i):
                    col_nickname = i
                if '姓名' in sheet1.cell_value(0,i):
                    col_realname = i
                if '学号' in sheet1.cell_value(0,i):
                    col_id = i
                if '总成绩' in sheet1.cell_value(0,i):
                    col_score = i
            if 'col_nickname' in dir() and 'col_realname' in dir() and 'col_id' in dir() and 'col_score' in dir():
                pass
            else:
                return render(request, 'once.html', {'msg':'错误,数据文件不包含关键信息!','class_id':class_id})
            for i in range(1, length1):
                # 如果是认证过的信息
                stu_id = sheet1.cell_value(i, col_id)
                if (stu_id in stuDict and sheet1.cell_value(i, col_realname)==stuDict[stu_id]) or stu_id in recode:
                    if stu_id in stuDict:
                        recode[stu_id]=sheet1.cell_value(i, col_score)
                        del stuDict[stu_id]
                    else:
                        if recode[stu_id]<sheet1.cell_value(i, col_score):
                            recode[stu_id]=sheet1.cell_value(i, col_score)
                # 利用真实姓名去反查学号
                realname = sheet1.cell_value(i, col_realname)
                for stu_id,stu_name in stuDict.copy().items():
                    if realname==stu_name and stu_id in sheet1.cell_value(i, col_nickname):
                        if stu_id in stuDict:
                            recode[stu_id]=sheet1.cell_value(i, col_score)
                            del stuDict[stu_id]
                        else:
                            if recode[stu_id]<sheet1.cell_value(i, col_score):
                                recode[stu_id]=sheet1.cell_value(i, col_score)
            # 最后处理既没有认证也没有真实姓名的学生对上传文档进行暴力搜索
            for stu_id,stu_name in stuDict.copy().items():
                for i in range(1, length1):
                    if stu_id in sheet1.cell_value(i, col_nickname) and stu_name in sheet1.cell_value(i, col_nickname):
                        if stu_id in stuDict:
                            recode[stu_id]=sheet1.cell_value(i, col_score)
                            del stuDict[stu_id]
                        else:
        ...
        # 上传平时成绩需要的文档
        elif request.FILES.get('sourceExcel'):
            if os.path.isfile(USER_FILE_DIR+'mooc/'+tea_name+'/'+classname+'.xls'):
                ...
                cols = 0
            uploadDir1 = USER_FILE_DIR+'moocData/'+tea_name #上传到的路径
            if not isdir(uploadDir1):
                mkdir(uploadDir1)
            uploadedFile1 = request.FILES.get('sourceExcel')
            if not uploadedFile1:
                return render(request, 'once.html', {'msg1':'错误,没有找到mooc数据文件!','nums':cols, 'classes':classes,'class_id':class_id})
            PATHs = join(uploadDir1, uploadedFile1.name)
            with open(PATHs, 'wb') as fp1:
                for chunk in uploadedFile1.chunks():
                    fp1.write(chunk)
            if request.FILES.get('targetExcel'):
                uploadDir2 = USER_FILE_DIR+'moocData/'+tea_name #上传到的路径
                if not isdir(uploadDir2):
                    mkdir(uploadDir2)
                uploadedFile2 = request.FILES.get('targetExcel')
                if not uploadedFile2:
                    return render(request, 'once.html', {'msg1':'错误,没有找到目标学生名单','nums':cols, 'classes':classes,'class_id':class_id})
                PATHt = join(uploadDir2, uploadedFile2.name)
                with open(PATHt, 'wb') as fp2:
                    for chunk in uploadedFile2.chunks():
                        fp2.write(chunk)
                PATHr = USER_FILE_DIR+'moocData/'+tea_name+'/'+classname+'.xlsx'

                deal = ExcelDealer(PATHs, PATHt, PATHr)
                stas = deal.run()
                if stas == "nfs":
                    return render(request, 'once.html', {'msg1':'错误,慕课数据文件未找到','class_id':class_id,'nums':cols, 'classes':classes})
                if stas == "nft":
                    return render(request, 'once.html', {'msg1':'错误,目标学生名单未找到','nums':cols, 'classes':classes,'class_id':class_id})
                if stas == "nfa":
                    return render(request, 'once.html',{'msg1':'错误,未找到任何文件!','nums':cols, 'classes':classes, 'class_id':class_id})
                if stas == "suc":
                    return render(request, 'once.html', {'msg1_s':'数据整理完成!','downloadDir':PATHr,'filename':classname + '.xlsx','nums':cols, 'classes':classes,'class_id':class_id})
            else:
                return render(request, 'once.html',{'msg1':'错误,未选择目标学生名单!','nums':cols, 'classes':classes, 'class_id':class_id})
        elif request.FILES.get("targetExcel"):
            if os.path.isfile(USER_FILE_DIR+'mooc/'+tea_name+'/'+classname+'.xls'):
                data = xlrd.open_workbook(USER_FILE_DIR+'mooc/'+tea_name+'/'+classname+'.xls')
                sheet = data.sheet_by_index(0)
                cols = sheet.ncols
                ...
                cols = 0
            return render(request, 'once.html',{'msg1':'错误,未选择慕课数据文件!','nums':cols, 'classes':classes, 'class_id':class_id})
        else:
            if os.path.isfile(USER_FILE_DIR+'mooc/'+tea_name+'/'+classname+'.xls'):
                data = xlrd.open_workbook(USER_FILE_DIR+'mooc/'+tea_name+'/'+classname+'.xls')
                sheet = data.sheet_by_index(0)
                ...
                cols = 0
            return render(request, 'once.html',{'msg1':'错误,未选择任何文件!','nums':cols, 'classes':classes, 'class_id':class_id})
    else:
        if os.path.isfile(USER_FILE_DIR+'mooc/'+tea_name+'/'+classname+'.xls'):
            data = xlrd.open_workbook(USER_FILE_DIR+'mooc/'+tea_name+'/'+classname+'.xls')
            sheet = data.sheet_by_index(0)
    ...
    for student in banji[0].students.all():
        ...
        for row in range(0, r_length):
            if student.id_num in sheet2.cell_value(row, 0):
                ...
                s_score=[0 if i=='无'or i=='-' else i for i in s_score]
                s_score=[float(x) for x in s_score]
                s_score=list(map(lambda x,y:x/y if y!=0 else 0,s_score,maxscore))
                x1=x2=x3=x4=0
                for score in s_score:
                    if score==1:
                        x1+=1
...
def download_result(request):
    ...
    downloadDir = request.POST['downloadDir']
    filename = request.POST['filename']
    response =FileResponse(file_iterator(downloadDir))
    response['Content-Type']='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    response['Content-Disposition']='attachment;filename={}'.format(escape_uri_path(filename))
    return response

@login_required()
def get_sign_dtl(request):
    """
    获取用户签到详情
    """
    json_data = {}
    banjis = BanJi.objects.filter(id=request.GET['className'])
    checked = []
    for banji in banjis:
        timeNow = datetime.datetime.now()
        events = Event.objects.filter(banji=banji,started_time__lte=timeNow).order_by('started_time')
        for event in events:
            begin = event.started_time
            end = event.closed_time
            id = event.id
            sign = Sign.objects.filter(event=event,user=request.GET['studentId'])
            if len(sign)>0:
                if sign[0].type_of==0:
                    status = '已签到'
                ...
                status = '未签到'
            checked.append({'name':banji.name,'begin':begin.strftime('%Y-%m-%d %H:%M:%S'),'end':end.strftime('%Y-%m-%d %H:%M:%S'),'status':status,'id':id})
        json_data['total'] = len(events)
    json_data['rows'] = checked
    return JsonResponse(json_data)

@login_required()
def get_homework_result(request):
    """
    获取用户已经完成的作业记录
    :param request: 请求
    :return: 含有用户已完成作业的json
    ...
    if request.GET['classname'] != '0':
        banji = BanJi.objects.get(pk=request.GET['classname'])
        if banji.work_result is None or banji.work_result=="":
            return JsonResponse(dict())
        else:
            return JsonResponse(eval(banji.work_result))
    else:
        return JsonResponse(dict())

@login_required()
def get_score_config(request):
    """
    获取班级的成绩比例
    """
    if request.user.isTeacher() and 'id' in request.GET.dict():
        banji = BanJi.objects.get(pk=request.GET['id'])
        return JsonResponse(eval(banji.score_weight))
    else:
        return JsonResponse(dict())

@login_required()
def save_score_config(request):
    if request.user.isTeacher() and 'id' in request.POST.dict():
        banji = BanJi.objects.get(pk=request.POST['id'])
        if request.user == banji.teacher or request.user.is_admin:
            temp = request.POST.dict()
            if "csrfmiddlewaretoken" in temp:
                del temp["csrfmiddlewaretoken"]
            if "id" in temp:
                del temp["id"]
            banji.score_weight=temp
            banji.save()
...
def get_update_time(request):
    ...
    if request.user.isTeacher() and 'id' in request.GET.dict():
        banji = BanJi.objects.get(pk=request.GET['id'])
        return JsonResponse({'update_time':banji.update_time.strftime("%Y-%m-%d %H:%M:%S")})
    else:
        return JsonResponse(dict())

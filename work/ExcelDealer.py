...
class ExcelDealer:
    def __init__(self, path_s, path_t, path_r):
        ...
        # 数据源文件预打开
        self.pat_s = ['.*学号', '.*昵称', '.*姓名', '(线上){0,1}总成绩$']
        ...
        try:
            ...
            for i in range(self.row_start_t, self.row_end_t):
                ...
                for col in range(1, self.ts.max_column + 1):
                    for j in range(5):
                        if re.search(self.pat_t[j], str(self.ts[get_column_letter(col) + str(i)].value)):
                            rec[j] = [i, col]
    ...
    def get_data_s(self):
        """慕课数据文件信息提取"""
        # print(self.nickname_col,self.real_name_col,self.id_col_s,self.score_col)
        for row in range(self.row_start_s, self.row_end_s):
            ...
            if self.is_xls_s:
              try:
                ...
                if self.ss.cell_value(row, self.score_col)!='-':
                  score = int(float(self.ss.cell_value(row, self.score_col)) + 0.5)
                else:
                  score = '-'
              except TypeError:
                print(row,self.nickname_col,self.real_name_col,self.id_col_s,self.score_col)
                raise
            else:
              try:
                ...
                if self.ss[get_column_letter(self.score_col) + row_str].value!='-':
                  score = int(float(self.ss[get_column_letter(self.score_col) + row_str].value) + 0.5)
                else:
                  score = '-'
              except TypeError:
                print(row_str)
    ...
    def find(self):
        ...
        for stu in self.target_data:
            count += 1
            # print("stu({}/{}):{}".format(count,total,self.target_data[stu]['Id']))
            stuInfo[self.target_data[stu]['Id']]={"rowId":stu,"realName":self.target_data[stu]['realName']}
            self.unsure_data[stu] = []
        queue = list(stuInfo.keys())

        for row in self.source_data:
            # counter += 1
            # sys.stdout.write("\r" + str(counter) + "/" + str(len(self.source_data)))
            level = 0
            if self.source_data[row]['Id']!="" and self.source_data[row]['Id'] in stuInfo.keys():
                # print("1000",self.source_data[row]['Id'],"学号在stuInfo中可以查到")
                level += 1000
                xuehao = self.source_data[row]['Id']
                if re.search(xuehao, self.source_data[row]['nickname']):
                    # print("100",self.source_data[row]['Id'],"学号在昵称中可以查到")
                    level += 100
                if stuInfo[xuehao]['realName'] == self.source_data[row]['realName']:
                    # print("10:",self.source_data[row]['realName'],"真实姓名相匹配")
                    level += 10
                    if re.search(stuInfo[xuehao]['realName'], self.source_data[row]['nickname']):
                        # print("1:",stuInfo[xuehao]['realName'],"真实姓名在昵称中可以查到")
                        level += 1
                        if xuehao in queue:
                            queue.remove(xuehao)
            # todo
            if level > 0:
                temp = self.source_data[row].copy()
                temp['level'] = level
                stu = stuInfo[self.source_data[row]['Id']]["rowId"]
        ...
        # 对剩下的信息进行暴力搜索
        for xuehao in queue:
            name = stuInfo[xuehao]['realName']
            for row in self.source_data:
                level = 0
                if re.search(xuehao, self.source_data[row]['nickname']):
                    # print("100",xuehao,"学号在昵称中可以查到")
                    level += 100
                if name == self.source_data[row]['realName']:
                    # print("10:",name,"真实姓名相匹配")
                    level += 10 
                if re.search(name, self.source_data[row]['nickname']):
                    # print("1:",name,"真实姓名在昵称中可以查到")
                    level += 1
                if level > 0:
                    temp = self.source_data[row].copy()
                    temp['level'] = level
                    try:
                        stu = stuInfo[xuehao]["rowId"]
                    except:
                        # print(stuInfo[xuehao])
                        raise
                    self.unsure_data[stu].append(temp)
                    if level > self.target_data[stu]['level']:
                        self.target_data[stu]['score'] = self.source_data[row]['score']
    ...
    def out_put(self):
        ...
        for row in range(self.row_start_t + 1, len(self.target_data) + 1 + self.row_start_t) if self.is_xls_t else range(self.row_start_t, len(self.target_data) + self.row_start_t):
            row_str1 = str(row)
            row_str = str(row - self.row_start_t + 1) if self.is_xls_t else str(row - self.row_start_t + 2)
            """打印每列数据"""
            ...
            comment = ""
            check = 0
            score_h = self.target_data[row_str1]['score'] if type(self.target_data[row_str1]['score'])!=str else 0
            self.unsure_data[row_str1].sort(reverse=True, key=get_key)
            for item in self.unsure_data[row_str1]:
                comment_temp = "昵称：" + item['nickname'] + " 真实姓名：" + item['realName'] + " 学号：" + item['Id'] + " 成绩：" + str(item['score']) + " 符合程度：" + str(item['level'])
                if comment_temp not in comment:
                    comment += comment_temp + "\n"
                if item['level'] >= 100:
                    if type(item['score']) == int and item['score'] > score_h:
                        score_h = item['score']
    ...
    def log_out_put(self):
        """日志信息输出"""
        # print("\n ")
        for it in self.target_data:
            # print(self.target_data[it])
            pass
        for it in self.unsure_data:
            # print(self.unsure_data[it])
            pass
    ...
    def run(self):
        if self.log:
            ...
            def newwork(o):
                ...
                o.out_put()
            newwork(self)
            # _thread.start_new_thread(newwork, (self,))
            # print("\ndone!")
            return "suc"
...
class Application(Frame):
    def __init__(self, master=None, **kw):
        ...
        self.PATHT = StringVar()
        self.PATHR = StringVar()
        self.pack()
        self.create_widgets()

    def create_widgets(self):
        # 退出按钮
        self.Quit = Button(self, text="退出", command=self.master.destroy)
        self.Quit.grid(row=3, column=1, sticky=NSEW)

        # 源文件文字显示
        self.sourceLabel = Label(self, text="慕课数据文件路径：", width=20, height=2)
        self.sourceLabel.grid(row=0, column=0, sticky=NSEW)

        # 原文件路径输入框
        self.sourcePath = Entry(self, textvariable=self.PATHS)
        self.sourcePath.grid(row=0, column=1, sticky=EW)

        # 名单文字显示
        self.targetLabel = Label(self, text="正方学生名单路径：", width=20, height=2)
        self.targetLabel.grid(row=1, column=0, sticky=EW)

        # 名单路径输入框
        self.sourcePath = Entry(self, textvariable=self.PATHT)
        self.sourcePath.grid(row=1, column=1, sticky=EW)

        # 结果文字显示
        self.resultLabel = Label(self, text="处理结果存放路径：", width=20, height=2)
        self.resultLabel.grid(row=2, column=0, sticky=EW)

        # 结果路径输入框
        self.resultePath = Entry(self, textvariable=self.PATHR)
        self.resultePath.grid(row=2, column=1, sticky=EW)

        # 确定按钮
        self.comfirm = Button(self, text="确定", command=self.deal_excel)
        self.comfirm.grid(row=3, column=0, sticky=NSEW)

        # 源文件选择
        self.previewBtnT = Button(self, text="选择文件", command=self.getPathT)
        self.previewBtnT.grid(row=1, column=2, sticky=EW)

        # 名单文件选择
        self.previewBtnS = Button(self, text="选择文件", command=self.getPathS)
        self.previewBtnS.grid(row=0, column=2, sticky=EW)

        # 存放位置选择
        self.previewBtnR = Button(self, text="选择路径", command=self.getPathR)
        self.previewBtnR.grid(row=2, column=2, sticky=EW)

        # 帮助说明按钮
        self.helpTextBtn = Button(self, text="帮助说明", command=self.helpText)
        self.helpTextBtn.grid(row=3, column=2, sticky=EW)

    def deal_excel(self):
        if self.PATHR.get() == "":
            messagebox.showinfo("提示", "您未填写存放路径，将默认与本程序存放在同一目录")
        deal = ExcelDealer(self.PATHS.get(), self.PATHT.get(), self.PATHR.get())
        deal.run()

    def getPathT(self):
        """源文件路径"""
        self.PATHT.set(
            askopenfilename(title="选择文件路径", initialdir="C:/Users/86159/Desktop/", filetypes=[("Excel", ".xlsx;*.xls")]))
    ...
    def getPathR(self):
        """结果存放路径"""
        self.PATHR.set(askdirectory(title="选择结果保存路径", initialdir="C:/Users/86159/Desktop/"))

    def helpText(self):
        """帮助说明"""
        content = "本程序用于根据提供的学生名单从慕课数据文件中筛选出相应学生平时成绩" \
                  "如果遇到无法解决的问题，请联系薛景老师。"
        messagebox.showinfo("帮助说明", content)


if __name__ == '__main__':
    root = Tk()
    root.title("自动筛选校内学生平时成绩v5.0")
    root.geometry("360x160+800+400")
    root.maxsize(360, 160)

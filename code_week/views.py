...
from .models import *
from .forms import *
from .choices import MAXNUMBER_CHOICES,MAXNUMBER_ONE_PROBLEM
import datetime, hashlib
from django.contrib.auth.decorators import permission_required, login_required
from django.shortcuts import redirect, get_object_or_404
from django.core.urlresolvers import reverse
from auth_system.models import MyUser, Group
from django.http import HttpResponse, JsonResponse, StreamingHttpResponse, HttpResponseRedirect, Http404
...
def course_list_for_teacher(request):
    ...
    previousCourses = CodeWeekClass.objects.filter(  # 过滤获取已经结束的课程
        ...
        end_time__lt=nowtime).order_by('-end_time')
    futureCourses = CodeWeekClass.objects.filter(    # 过滤获取还未开始的课程
        teacher=request.user).filter(
        begin_time__gt=nowtime).order_by('-end_time')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            newCourse = CodeWeekClass(name=form.cleaned_data['course_name'],
                                      ...
                                      numberEachProblem=form.cleaned_data['maxeachproblem'])
            newCourse.save()
            # 添加题目
...
def student_view_course(request, courseId):
    ...
    if course.count() == 0:
        ...
        for item in MAXNUMBER_ONE_PROBLEM:
            if item[0] == course[0].numberEachProblem:
                numberEachProblem = item[1]
        for item in MAXNUMBER_CHOICES:
            if item[0] == course[0].numberEachGroup:
                numberEachGroup = item[1]
        return render(request, 'code_week/student_course_detail.html', {'course' : course[0], 'problem': problem,
                'numberEachProblem': numberEachProblem, 'numberEachGroup': numberEachGroup})
    else:
        raise Http404()
...
def teacher_course_info(request, courseId):
    ...
    result = {'name':course.name, 'number':course.numberEachGroup,
              'eachproblem':course.numberEachProblem,
...
def tarFiles(courseId, className, teacherName, course_type):
    ...
    os.mkdir(workDir)
    # os.chdir(workDir)
    studentDir = os.path.join(workDir, "2、学生报告（需要电子签名）及代码")
    os.mkdir(studentDir)
    os.chdir(studentDir)
    course = None
    totalStu = 0
    ...
    if course.numberEachGroup == 1:  # 单人模式
        ...
        f.save(os.path.join(workDir, '3、记分册.xls'))
        index = 1
        for group in groups:
            NoStr = str(index)
            if len(NoStr) == 1:
    ...
    docsDir = os.path.join(workDir, '4、实验指导书')
    os.mkdir(docsDir)
    toCopyFiles = os.listdir(os.path.join(USER_FILE_DIR, 'documents'))
    for file in toCopyFiles:
        if "5、" in file:
            shutil.copy(os.path.join(USER_FILE_DIR, 'documents', file), os.path.join(workDir, file))
        else:
    ...
    # 23-24-1-算法与数据结构设计（B0364090C）-B210305（35人）-戴华
    year = int(course.begin_time.strftime("%Y"))
    if "02"<=course.begin_time.strftime("%m")<="07":
        xueqi = str(year-1)[-2:] + "-" + str(year)[-2:] + "-2"
    elif course.begin_time.strftime("%m")=="01":
        xueqi = str(year-1)[-2:] + "-" + str(year)[-2:] + "-1"
    else:
        xueqi = str(year)[-2:] + "-" + str(year+1)[-2:] + "-1"
    targetName = xueqi + "-程序设计(上机) (" + course_type + ")-" + className + "(" + str(totalStu) + "人)" + '-' + teacherName
...
def delete_record(request):
    if request.method == 'POST':
        ...
        try:
            # 在此处添加删除记录的逻辑
            CodeDirHistory.objects.filter(id=record_id).delete()
...
def save_record(request):
    if request.method == 'POST':
        try:
            ...
            ratings_str = str(ratings).replace('\'', '').replace(': ', ':').replace(', ', ',').strip('{}')
            # 在这里处理保存记录的逻辑
            # 例如，更新数据库中的记录
            CodeDirHistory.objects.filter(id=record_id).update(contribution=ratings_str)

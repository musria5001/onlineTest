...
def change_email(request):
    if request.method == 'POST':
        ...
        if cache.get(ip):
            errors.append('请不要频繁发送邮件')
        elif email!=None:
            cache.set(ip, 1, 60)  # 60秒内同一IP只能发送一次邮件
            user = request.user
            token_generator = default_token_generator
            from_email = settings.EMAIL_HOST_USER
            current_site = get_current_site(request)
...
def create_users(request):
    ...
    try:
        ...
        try:
            ...
            user.set_password(id_num+'@Njupt')
            user.save()
...
def update_user(request):
    ...
    return render(request, 'update_user.html',
                  context={'myuser': user, 'groups': groups, 'current_group_id': current_group_id})
...
def reset_user(request, pk):
    ...
    if user.school_short is None:
        user.set_password(user.id_num + '@Njupt')
    else:
        user.set_password(user.id_num + '@' + user.school_short.title())
    user.save()
    return redirect(reverse('user_list'))

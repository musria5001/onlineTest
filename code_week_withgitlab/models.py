...
from .choices import MAXNUMBER_CHOICES,MAXNUMBER_PROBLEM
...
class CodeWeekClass(models.Model):
    ...
    numberEachProblem = models.IntegerField(choices=MAXNUMBER_PROBLEM, default=3)
...
class CodeWeekClassGroup(models.Model):
    ...
    counter = models.IntegerField(default=0)
    using = models.BooleanField(default=True)    # 用来标识小组是否使用,如果解散分组会将这个设为False
    nowCodeDir = models.ForeignKey("CodeDirHistory", null=True, related_name="NowCodeZip")
    mergeCodeDir = models.ForeignKey("CodeDirHistory", null=True , related_name="MergeCodeZip", on_delete=models.SET_NULL) # 用于处理MergeRequest的合并下载问题
...
class CodeDirHistory(models.Model):
    ...
    contribution = models.TextField("贡献度",null=True)
    # newContribution = models.TextField("贡献度划分")
    submitTime = models.DateTimeField(auto_now_add=True)
    fileHash = models.CharField("文件hash", max_length=40)
    isUsing = models.BooleanField("是否删除",default=True)

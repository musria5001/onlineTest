        ...
        if action == 'c':   # 创建新分组（成为组长）
            ...
            if student.group: # 已经加入组了
                ...
                try:
                    with transaction.atomic():
                        ...
                        user = gl.users.list(username = str(student.get_idnum()))[0]
                        ...
                        try:
                            ...
                            sendMsgToStudent(message, {'msg': 'fail','info': '服务器响应超时，请稍后重试','action': 'c'})
                            return
                        except Exception as e:
                            if str(e)=='400: Branch already exists':
                                pass
                            else:
                                raise
                    ...
                    sendMsgToStudent(message, {'msg': 'fail','info': traceback.format_exc(),'action': 'c'})
                    ...
                    try:
                        with transaction.atomic():
                            ...
                            try:
                                gl = gitlab.Gitlab.from_config('somewhere',['/etc/python-gitlab.cfg'])
                                try:
                                    name = str(courseId) + '_' + str(groupid)
                                    project = gl.projects.list(search = name)[0]
                                    user = gl.users.list(username = str(student.get_idnum()))[0]
                                    member = project.members.create({'user_id': user.id, 'access_level':gitlab.DEVELOPER_ACCESS})
                                    try:
                                        branch = project.branches.create({'branch': str(student.get_idnum()),'ref':'master'})
                                    except Exception as e:
                                        sendMsgToStudent(message, {'msg': 'fail','info': traceback.format_exc(),'action': 'j'})
                                        return
                                except Exception as e:
                                  if str(e)=='409: Member already exists':
                                    pass
                                  else:
                                    sendMsgToStudent(message, {'msg': 'fail','info': traceback.format_exc(),'action': 'j'})
                                    return
            ...
            if student.group:
                if not student.isLeader:  # 不是组长
                    ...
                    sendMsgToStudent(message, result)
                    return
                if student.group.selectedProblem: # 已经选择了题目
                    result = {'msg': 'fail', 'info': '您已经选择过题目'}
                    sendMsgToStudent(message, result)
                    return
                else:
                    problem = None
                    try:
                        ...
                        problemGroup = CodeWeekClass.objects.get(pk=courseId)
                    except:
                    ...
                    for group in allGroup:
                        if group.selectedProblem == problem:
                            number += 1
                    if number >= problemGroup.numberEachProblem:
                        result = {'msg': 'fail', 'info': '这个题目已经有4个小组选择了，请选择别的题目'}
                        sendMsgToStudent(message, result)
                        return
                    result = None
                    infoObject = None
                    try:
                        with transaction.atomic():
                            ...
                            course.counter += 1
                            course.save()
                            result = {'action': 'choose', 'problemId': problem.problem_id, 'title': problem.title,

...
def course_list_for_teacher(request):
    ...
    previousCourses = CodeWeekClass.objects.filter(  # 过滤获取已经结束的课程
        ...
        end_time__lt=nowtime).order_by("-begin_time")
    futureCourses = CodeWeekClass.objects.filter(    # 过滤获取还未开始的课程
        teacher=request.user).filter(
        begin_time__gt=nowtime)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            newCourse = CodeWeekClass(name=form.cleaned_data['course_name'],
                                      ...
                                      numberEachProblem=form.cleaned_data['maxgroup'])
            newCourse.save()
            # 添加题目
            ...
            # 添加学生
            try:
                gl = gitlab.Gitlab.from_config('somewhere',['/etc/python-gitlab.cfg'])
            except:
                return render(request, 'warning.html', {'info': 'Gitlab连接错误'})
            for stu_detail in request.POST['students'].splitlines():
                if len(stu_detail.split()) > 1:
                    ...
                    students = MyUser.objects.filter(id_num=id_num)
                    if len(students)>0:
                        student = students[0]
                    else:
                        try:
                    ...
                    try:
                        ...
                        user = gl.users.list(username= str(student.id_num) )[0]
                        user.email = str(id_num) + '@njupt.edu.cn' 
                        user.username = id_num
                        user.name = student.username        
                        user.save()
                    except (ObjectDoesNotExist,IndexError) as e:
                        user = gl.users.create({'email':str(id_num) + '@njupt.edu.cn', 'password':id_num, 'username':id_num, 'name':student.username, 'skip_confirmation':True})
                    if newCourse.students.all().filter(codeweekclassstudent__student=student).count() == 0: # 判断学生是否已经存在
                        if form.cleaned_data['maxnumber'] == "1":  # 没有分组操作，直接给每个学生加入一个只有自己一个人的组
                            ...
                            name = str(newCourse.id) + '_' + str(newgroup.id)
                            try:
                                user_project = user.projects.create({'name': name})
                                project = gl.projects.get(user_project.get_id())
                                try:
                                    branch = project.branches.create({'branch': 'master','ref':'master'})
                                    pass
                                except:
                                    pass
                            except:
                        ...
                        if newCourse.students.all().filter(codeweekclassstudent__student=student).count() == 0:
                            if form.cleaned_data['maxnumber'] == "1":  # 没有分组操作，直接给每个学生加入一个只有自己一个人的组
                                ...
                                newCourseStudent = CodeWeekClassStudent(codeWeekClass=newCourse, student=student,
                                                                        ...
                                                                        isLeader=True)
                                newCourseStudent.save()
                                name = newCourse.id + '_' +newgroup.id
                                try:
                                    user_project = user.projects.create({'name': name})
                                    project = gl.projects.get(user_project.get_id())
                                    try:
...
def teacher_course_info(request, courseId):
    ...
    result = {'name':course.name, 'number':course.numberEachGroup,
              ...
              'groupnumber': course.numberEachProblem}
        ...
        if form.is_valid():
            ...
            for stu_detail in request.POST['students'].splitlines():
                if len(stu_detail.split()) > 1:
                    id_num, username = stu_detail.split()[0], stu_detail.split()[1]
                    if len(id_num)<8:
                        return render(request, 'warning.html', context={'info': '学号（'+str(id_num)+'）错误：学号不能少于8位！'})
                    try:
                        student = MyUser.objects.get(id_num=id_num)
                    except:
                        student = MyUser(id_num=id_num, email=id_num + '@njupt.edu.cn', username=username)
                    ...
                    if course.students.all().filter(codeweekclassstudent__student=student).count() == 0:
                        if course.numberEachGroup == 1:  # 没有分组操作，直接给每个学生加入一个只有自己一个人的组
                            ...
                            newCourseStudent = CodeWeekClassStudent(codeWeekClass=course, student=student,
                                                                    ...
                                                                    isLeader=True)
                            newCourseStudent.save()
                            name = str(courseId) + '_' + str(newgroup.id)
                            try:
                                user_project = user.projects.create({'name': name})
                                project = gl.projects.get(user_project.get_id())
                                try:
...
@login_required
def dealMergeRequest(request, courseId):
    student = None
    try:
    ...
    if request.method == 'POST':
        ...
        # print(request.POST.get("mergerequest"))
        # print(request.POST.get("contribution"))
        if form.is_valid():
            try:
                gl = gitlab.Gitlab.from_config('somewhere',['/etc/python-gitlab.cfg'])
            except:
                return render(request, 'warning.html', {'info': 'gitlab初始化错误'})
            if request.POST["mergerequest"] == "accpet":
                project_name = courseId+"_"+groupId
                project = gl.projects.list(search = project_name)[0]
                mr = project.mergerequests.list()[0]
                try:
                    mr.merge()
                    items = CodeDirHistory.objects.filter(group = groupId, contribution = "TBD", isUsing = True)
                    for item in items:
                        item.contribution = request.POST["contribution"]
                        item.save()
                    group.nowCodeDir = group.mergeCodeDir
                    group.mergeCodeDir = None
                    group.save()
                    infoText = {'action': 'merge','leader': student.get_full_name(), 'id': group.nowCodeDir.id, 'groupId': student.group.id}
                    ...
                    return render(request, 'warning.html', {'info': '检查合并请求！拒绝没有变动的合并请求。'})
                return HttpResponseRedirect(reverse('submit_code', args=(student.codeWeekClass.id,)))
            elif request.POST["mergerequest"] == "refuse":
                items = CodeDirHistory.objects.filter(group = groupId, contribution = "TBD", isUsing = True)
                for item in items:
                    item.isUsing = False
                    item.save()
                ...
                mr = project.mergerequests.list()[0]
                mr.delete()
                infoText = {'action': 'refuse_merge','leader': student.get_full_name(), 'groupId': student.group.id}
                infoObject = LatestInfo.objects.create(course=student.codeWeekClass, info=json.dumps(infoText))
                updateLatestInfo(courseId, infoObject)
                return HttpResponseRedirect(reverse('submit_code', args=(student.codeWeekClass.id,)))
            else:
                return render(request, 'warning.html', {'info': '发生错误1'})
        else:
            return render(request, 'warning.html', {'info': '发生错误2'})
@login_required
...
def submit_code(request, courseId):
    ...
    try:
        ...
        return render(request, 'warning.html', {'info': 'Gitlab连接失败'})

    groupId = str(group.id)

    difflist = get_members_mrs_diff(courseId,groupId)
        ...
        # 检查是否是组长，现在的逻辑是只有组长才能上传代码，而且需要组长手动填写贡献量
            if not student.isLeader:
                ...
                if not file.name.endswith(".zip"):
                    return render(request, 'warning.html', {'info': '只支持zip压缩文件，你提交的文件名为{}'.format(file.name)})
                else:
                    ...
                    try:
                        ...
                        return render(request, 'warning.html', {'info': '压缩文件格式不正确，请尝试使用其他软件进行压缩'})
                id_num = request.user.id_num
                username = request.user.id_num
                file_path = filename+"_files"
                projectId = courseId+"_"+str(group.id)
                project = gl.projects.list(search=projectId)
                if project:
                    project = project[0]
                else:
                    return render(request, 'warning.html', {'info': '请等待组长进行第一次提交！'})
                if not student.group.nowCodeDir:
                    return render(request, 'warning.html', {'info': '请等待组长进行第一次提交！'})
                if get_mrs_count(courseId,groupId) == -1:
                    return render(request, 'warning.html', {'info': '连接服务器失败，请稍后再试！'})
                elif get_mrs_count(courseId,groupId) != 0:
                    return render(request, 'warning.html', {'info': '提交失败，已存在一个合并请求，请提醒组长审核！'})
                leaderId = project.members.list()[0].username
                gitpath = "http://"+id_num+":"+id_num+"@10.22.243.75/git/"+leaderId+"/"+courseId+"_"+str(group.id)
                # commits1 = project.commits.list(ref_name=id_num)
                i = os.system("cd "+tempdir+"\n"
                        +"git clone "+gitpath+"\n"
                        +"cp -rf "+tempdir+"/"+courseId+"_"+str(group.id)+"/.git "+file_path+"\n"
                        +"cd "+file_path+"\n"
                        +"git checkout -b "+id_num +'\n'
                        +"git add --all\n"
                        +"git config user.name "+id_num+"\n"
                        +"git config user.email "+id_num+"@njupt.edu.cn\n"
                        +"git commit -m "+id_num+"\n"
                        +"git push --set-upstream -f origin "+ id_num +"\n"
                        +"rm -rf .git")
                time.sleep(1)
                # commits = project.commits.list(ref_name=id_num)
                # if len(commits) != len(commits1) + 1:
                    # return render(request, 'warning.html', {'info': 'Gitlab初始化有误，请联系老师重新添加学生账号！'})
                # 处理文件
                ...
                newCodeHistory = CodeDirHistory.objects.create(zipFile=newCodeZip, dirText=dir_result,
                                                                ...
                                                                group=student.group,
                                                                fileHash=sha1result)
                student.group.mergeCodeDir = newCodeHistory 
                student.group.save()
                infoText = {'action': 'submit_merge_code', 'user': student.get_full_name(), 'id': newCodeHistory.id, 'groupId': student.group.id}
                infoObject = LatestInfo.objects.create(course=student.codeWeekClass, info=json.dumps(infoText))
                updateLatestInfo(courseId, infoObject)
                try:
                    mr = project.mergerequests.create({'source_branch': id_num,
                                            'target_branch': 'master',
                                            'title': 'merge '+id_num+" to master",}) #创建一个MR            
                except:
                    pass
            ...
            # 保存序列化结果到数据库
            # 保存这个压缩文件，来方便打包文件的下载
            if get_mrs_count(courseId,groupId) > 0:
                return render(request, 'warning.html', {'info': '优先处理合并请求'})
            file = request.FILES['codeFile']
            if file.size > MAX_CODE_TAR_SIZE:
                return render(request, 'warning.html', {'info': '只支持小于5M大小的文件'})
            if not file.name.endswith(".zip"):
                ...
                try:
                    ...
                    shutil.rmtree(tempdir)
                    return render(request, 'warning.html', {'info': '压缩文件格式不正确，请尝试更换压缩软件重新压缩'})
                sha1result = sha1(filename)
                # 这边可以利用文件hash做一些额外工作，例如比较和以前版本是否一样
                try:
                    dir_result = copy_generage_dict_info(filename + '_files/', student.group)
                except UnicodeDecodeError:
                    return render(request, 'warning.html', {'info': '压缩文件编码错误，请尝试使用其他的压缩软件进行文件打包'})
                # 保存整个压缩文件
                newCodeZip = CodeZipFile.objects.create(fileName = file.name)
                shutil.copy(filename, codeZipFileName(newCodeZip.id))
                #获取解压文件夹
                ...
                username = request.user.id_num
                file_path = filename+"_files"
                gitpath = "http://"+id_num+":"+id_num+"@10.22.243.75/git/"+id_num+"/"+courseId+"_"+str(group.id)
                try:
                    gl = gitlab.Gitlab.from_config('somewhere',['/etc/python-gitlab.cfg'])
                    projectId = courseId+"_"+str(group.id)
                    project = gl.projects.list(search=projectId)[0]
                    commits1 = project.commits.list()
                    i = os.system("cd "+tempdir+"\n"
                            +"git clone "+gitpath+"\n"
                            +"cp -rf "+tempdir+"/"+courseId+"_"+str(group.id)+"/.git "+file_path+"\n"
                            +"cd "+file_path+"\n"
                    ...
                    return render(request, 'warning.html', {'info': 'Gitlab初始化有误，请重新建立小组！'})          
                # 删除产生的文件
                shutil.rmtree(tempdir)
                # pdb.set_trace()
                # 保存代码版本（目录序列化结果）
...
def handle_upload_report(request, courseId):
    ...
    if request.method == 'POST':
        ...
        if not form.is_valid():
            return render(request, 'warning.html', {"info":"请选择正确的报告文件"})
        else:
...
def download_codeZip(request, historyId):
    ...
    if codeHistory: #能够匹配到历史代码,检查学生是否是这个代码的所有者
        ...
        try:
            ...
            return render(request, 'warning.html', {'info': '你无权下载该文件'})
        owner = False
...
# 用于教师打包所有材料
def tarFiles(courseId, className, teacherName, course_type):
    ...
    if course.numberEachGroup == 1:  # 单人模式
        ...
        sheet1.write(0, 2, '组别')
        sheet1.write(0, 3, '题目')
        sheet1.write(0, 4, '贡献度比例')
        sheet1.write(0, 5, '贡献度详情')
        appearStudents = set()
        allStudents = set()
        ...
        for group in groups:
            ...
            memberLineMap = {}
            contribution_sum = {}
            if group.selectedProblem is not None:
                p_title = group.selectedProblem.title
            else:
                p_title = ""
            for student in group.Group_member.order_by("-isLeader"):
                sheet1.write(memberNumber, 0, student.student.id_num)
                sheet1.write(memberNumber, 1, student.student.username)
                sheet1.write(memberNumber, 2, "{:02}".format(groupNumber))
                sheet1.write(memberNumber, 3, p_title)
                memberLineMap[student.get_full_name()] = memberNumber
                contribution_sum[student.get_full_name()] = 0.0
                memberNumber += 1
                appearStudents.add(student.get_full_name())
            index = 5 # 贡献度开始的列数
            count = 0
            for history in group.Code_history.all():
                if history.isUsing == True:
                    ...
                    for single_member_contribution in contribution_list:
                        try:
                            ...
                            contribution_sum[contribution_record[0]] += float(contribution_record[1].split('%')[0])
                            sheet1.write(memberLineMap[contribution_record[0]], index, float(contribution_record[1].split('%')[0]))
                        except KeyError:
                            pass
                    index += 1
                    count += 1
            for student in group.Group_member.all():
                ...
                sheet1.write(memberLineMap[student.get_full_name()], 4, contribution_average)
            groupNumber += 1
        # 记录未在分组中出现的学生
    ...
    # 薛景-2019-2020-1-程序设计(上机)(B0364073C)-B181207(36人)
    year = int(course.begin_time.strftime("%Y"))
    if "02"<=course.begin_time.strftime("%m")<="07":
        xueqi = str(year-1) + "-" + str(year) + "-2"
    elif course.begin_time.strftime("%m")=="01":
        xueqi = str(year-1) + "-" + str(year) + "-1"
    else:
        xueqi = str(year) + "-" + str(year+1) + "-1"
    targetName = teacherName + "-" + xueqi + "-程序设计(上机) (" + course_type + ")-" + className + "(" + str(totalStu) + "人)" 
    ...
    os.chdir("../")
    newTar = TarHistory.objects.create(course=course,filename=targetName+".zip")
...
def handelTeacherTar(request, courseId):
    ...
    if tarFiles(courseId, className, teacherName, course_type):
...
#用于获取用户的MR
def get_members_mrs_diff(courseId,groupId):
    try:
        gl = gitlab.Gitlab.from_config('somewhere',['/etc/python-gitlab.cfg'])
    except:
        return 
    projectId = courseId+"_"+groupId
    project = gl.projects.list(search=projectId)
    if project:
        project = project[0]
    else:
    ...
    # print(project)
    # print(mrs)
    # 筛选：把state不为opened的MR去掉
    difflist = []
    
    for mr1 in mrs:
        if mr1.state.strip() == 'opened':
            if not mr1.changes()['changes']:
                return ['组员提交的代码与原版本程序相同，请拒绝此次请求，并提示组员重新上传。']
            temp = ""
            for mr in mr1.changes()['changes']:
                try:
                    mr['old_path'] = mr['old_path'].encode('cp437').decode('gbk')
                    mr['new_path'] = mr['new_path'].encode('cp437').decode('gbk')
                except:
                    pass
                if mr['new_file']:
                    temp += "新建文件：{}<br>".format(mr['new_path'])
                elif mr['renamed_file']:
                    temp += "重命名文件：{}改为{}<br>".format(mr['old_path'],mr['new_path'])
                elif mr['deleted_file']:
                    temp += "删除文件：{}<br>".format(mr['old_path'])
                else:
                    temp += "修改文件：{}<br><pre>{}</pre>".format(mr['old_path'],mr['diff'])
            difflist.append(temp)
            break
    return difflist

def get_mrs_count(courseId,groupId):
    try:
        gl = gitlab.Gitlab.from_config('somewhere',['/etc/python-gitlab.cfg'])
    except:
        return -1 

...
def list_course(request,id):
    ...
    if user.is_admin == True or user.groups.all()[0].name == '老师':
        for week in weeks:
            ...
            for resource in resource_wt:
                if (resource.creater.is_admin == True)&(resource not in array_r):
                    array_r.append(resource)
                    array_r.sort(key = attrgetter('num'))
            if any(resource_wt) == True:
                array_wt.append(week)
        return render(request,'course_list.html', {'courser':courser,'weeks':array_wt,'resources':array_r})
...
def newlesson(request):
    return render(request, 'newlesson.html')